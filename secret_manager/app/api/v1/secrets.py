...
def store_object_storage_secret(secret: ObjectStorageSecret, user_id: str = Depends(get_current_user)):
    store_secret(user_id, secret)
    return {"message": "Secret stored successfully."}

@router.get("/secrets/object-storage")
def get_object_storage_secret(user_id: str = Depends(get_current_user)):
